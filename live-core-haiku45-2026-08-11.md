# LIVE core system prompt — Haiku 4.5 (claude-haiku-4-5-20251001), headless -p probe, 2026-08-11

Transcribed VERBATIM by Haiku 4.5 from its own live context (the behavioral system prompt that ships BEFORE tool descriptions, skill listings, agent-type listings, and deferred-tool declarations). Tool descriptions, skill listings, agent-type definitions, MCP server instructions, and session-injected system-reminder blocks are excluded per transcription scope. This probe runs under `-p` headless mode (non-interactive session).
...
>  - Additional working directories: <cwd>
>  - Platform: win32
...
NOTE: Fast-mode line is ABSENT in this session — only Opus-family models list fast-mode availability. Shell line is injected by harness (PowerShell primary on Windows); user's own preference (Git Bash primary per CLAUDE.md) overrides.
...
### Sections present: model-neutral core is byte-identical across all three models

| Section | Haiku 4.5 | Opus 4.6 | Fable 5 | Delta |
|---|---|---|---|---|
| C1–C15 (behavioral core) | ✓ Verbatim | ✓ Verbatim | ✓ Verbatim | **No delta** — core safety/action/privacy/copyright rules are model-neutral |
| C16–C17 (git/PR workflow) | ✓ Verbatim | ✓ Verbatim | ✓ Verbatim | **No delta** — injected identically into Bash tool descriptions |

### Model-specific differences in environment and availability

| Aspect | Haiku 4.5 | Opus 4.6 | Fable 5 | Note |
|---|---|---|---|---|
| Model ID line | `claude-haiku-4-5-20251001` | `claude-opus-4-6` | `claude-fable-5` | **Harness-injected per session** |
| Knowledge cutoff | February 2025 | May 2025 | January 2026 | **Model-specific; injected by harness** |
| Fast mode line | **ABSENT** | "available on Opus 5/4.8" | "available on Opus 5/4.8/4.7" | **Only Opus-family models** — Haiku 4.5 has no fast mode |

### Sections absent from Haiku vs. references

**Haiku 4.5 appears to lack a distinct autonomy paragraph**. The Fable reference (S9) includes a detailed "You are operating autonomously" section not appearing in the environment-conditionality notes above. This absence could be:
1. **Harness-conditional** — autonomy guidance may be injected selectively (desktop/interactive vs. headless `-p` mode)
2. **Model-specific trim** — Haiku may receive abbreviated autonomy guidance
3. **Not yet transcribed** — if present, it would appear after C11 but before safety-rule sections

Query status: C1–C17 confirmed verbatim; autonomy section status UNCONFIRMED — if present in my live context, it did not appear in the quoted sections above.

**All other sections** (System, Doing tasks, Executing actions, Tool usage, Tone, Text output, Session guidance, Environment, Scratchpad, Context, Instruction boundaries, Actions, Privacy, Copyright, Git) are **byte-identical** across Haiku 4.5, Opus 4.6, and Fable 5.

---

## Steering signals (self-report)

Haiku 4.5's prompt emphasizes **conciseness and efficiency** as primary steerage relative to Fable 5:

1. **Tone narrowing**: "Your responses should be short and concise" — explicit vs. Fable's implicit "brevity is good". No counterweighting emphasis on readability-over-compression; the tone section is smaller.

2. **Text output guidance is compressed**: The text-output section (C7) reiterates "Brief is good — silent is not. One sentence per update is almost always enough" and ends abruptly at pronoun guidance, omitting the richer "Being readable and being concise are different things" passage from the Fable reference (S4), which trades off speed for legibility.

3. **No fast mode** — Haiku 4.5 receives no fast-mode option at all (both Opus and Fable do), suggesting Haiku is already the speed-optimized tier and needs no trade-off mechanism.

4. **Knowledge cutoff is newest in Haiku** (Feb 2025) vs. Opus's May 2025 and Fable's Jan 2026. The prompt does NOT say "Haiku is smaller" or "Haiku is for quick tasks", so the steering is indirect: newer knowledge + pressure toward brevity + no capability-upgrade path signals Haiku as the tier for rapid, current-knowledge responses.

5. **Autonomy section status**: If absent, Haiku may receive less nudge toward independent action, consistent with a faster/simpler processing model. If present but not transcribed, the absence itself signals no unique autonomy steerage per model.

Overall steering read: **Haiku is steered toward brevity, speed, and direct answers** — the prompt omits richer guidance on readable-vs-concise tradeoffs and speeds through tone/text sections. Fable's prompt invests in nuance and legibility; Haiku's gets efficiency.
